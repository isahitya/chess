				...
				if blockslist[i][j].piece.team!=chance:
					...
					for each in oppMoves:
						if each[0]==kxpos and each[1]==kypos:
							if checkIfUnderAttack(i,j,chance)==False:
								check=True
	if check==False:return False
	kingMoves=king.calculateMoves(kxpos,kypos)
	if len(kingMoves)==0:return True#All possible moves are under attack
#	x=0				
#	l=len(kingMoves)-1
#	for each in oppMoves:		#Wrote this commmented code when I was not checking for if king's move under attack
#		if each[0]==kingMoves[x][0] and each[1]==kingMoves[x][1]:
#			x+=1
#			if(x>l):break
#	if(x>l):
#		return True
